...
impl SimpleScene
{
    pub fn new(base: &VkBase, allocator: &mut Allocator) -> SimpleScene {
        ...
        let frame_timer = [DrawableText::new(base, Vec3::new(-0.8, -0.8, 0.0), atlas_info, texture, allocator, "0000 ", 64)];
        ...
        for scene in scenes.iter_mut() {
            ...
            if !scene.initialized {
                ...
                unsafe {
                    ...
                    let size = scene.frame_timer[0].font_atlas.staging.size;
                    let data_ptr = base.device.logical.map_memory(scene.frame_timer[0].font_atlas.staging.memory, 0, size, vk::MemoryMapFlags::empty()).unwrap() as *mut u8;
                    data_ptr.copy_from_nonoverlapping(atlas.data.as_ptr(), size as usize);
                    base.device.logical.unmap_memory(scene.frame_timer[0].font_atlas.staging.memory);

					utils::image::transition_image_layout::<ImageLayout_Undefined, ImageLayout_ShaderReadOnlyOptimal>(&base.device, *cb, &scene.frame_timer[0].font_atlas);

                    utils::image::transition_image_layout::<ImageLayout_ShaderReadOnlyOptimal, ImageLayout_TransferDstOptimal>(&base.device, *cb, &scene.frame_timer[0].font_atlas);
                    utils::image::copy_buffer_to_image(&base.device, *cb, &scene.frame_timer[0].font_atlas, &scene.frame_timer[0].font_atlas.staging, atlas.w, atlas.h);
                    utils::image::transition_image_layout::<ImageLayout_TransferDstOptimal, ImageLayout_ShaderReadOnlyOptimal>(&base.device, *cb, &scene.frame_timer[0].font_atlas);
                }
