...
use ash::vk;
use stb_truetype::{get_font_atlas_path, parse_font_atlas_info};
...
impl SimpleScene
{
    pub fn new(base: &VkBase, allocator: &mut Allocator) -> SimpleScene {
        ...
        let atlas = farbfeld_image::load_ff_from_memory(FONT_ATLAS_DATA).expect("Could not find font atlas.");
		let atlas_info = parse_font_atlas_info(FONT_ATLAS_PATH).expect("Provide a valid font atlas generated by the local stb_truetype crate");
        let font_atlas = utils::image::create_texture_image(&base.device, atlas.w, atlas.h, (atlas.w * atlas.h * 4) as u64, PixelFormat::RGBA);
        let frame_timer = [DrawableText::new(base, Vec3::new(-0.8, -0.8, 0.0), atlas_info, allocator, "0000 ", 64)];
        DrawableText::init_font_atlas(&base.device, &font_atlas, &frame_timer);
        ...
        for scene in scenes.iter_mut() {
            ...
            if !scene.initialized {
                ...
                unsafe {
                    ...
                    let size = scene.font_atlas.staging.size;
                    let data_ptr = base.device.logical.map_memory(scene.font_atlas.staging.memory, 0, size, vk::MemoryMapFlags::empty()).unwrap() as *mut u8;
                    data_ptr.copy_from_nonoverlapping(atlas.data.as_ptr(), size as usize);
                    base.device.logical.unmap_memory(scene.font_atlas.staging.memory);

					utils::image::transition_image_layout::<ImageLayout_Undefined, ImageLayout_ShaderReadOnlyOptimal>(&base.device, *cb, &scene.font_atlas);
                    utils::image::transition_image_layout::<ImageLayout_ShaderReadOnlyOptimal, ImageLayout_TransferDstOptimal>(&base.device, *cb, &scene.font_atlas);
                    utils::image::copy_buffer_to_image(&base.device, *cb, &scene.font_atlas, &scene.font_atlas.staging, atlas.w, atlas.h);
                    utils::image::transition_image_layout::<ImageLayout_TransferDstOptimal, ImageLayout_ShaderReadOnlyOptimal>(&base.device, *cb, &scene.font_atlas);
                }
    ...
    pub fn release(scenes: &mut [Self], base: &VkBase) {
        for scene in scenes.iter_mut() {
            ...
            unsafe {
			    base.device.logical.destroy_buffer(scene.font_atlas.staging.buffer, None);
                base.device.logical.free_memory(scene.font_atlas.staging.memory, None);
                base.device.logical.destroy_image(scene.font_atlas.resource.image, None);
                base.device.logical.free_memory(scene.font_atlas.resource.memory, None);
                base.device.logical.destroy_image_view(scene.font_atlas.image_view, None);
                base.device.logical.destroy_sampler(scene.font_atlas.sampler, None);
            }
        }
    }
