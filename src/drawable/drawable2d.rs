        ...
        for mesh_bundle in mesh_bundles.iter_mut() {
            ...
            unsafe {
                ...
                if mesh_bundle.mesh.dirty_vertices {
                    let data_ptr = device.logical.map_memory(mesh_bundle.staging.memory, staging_offset, size_vrt, vk::MemoryMapFlags::empty()).unwrap() as *mut [f32; 2];
                ...
                if mesh_bundle.mesh.dirty_colour {
                    let data_ptr = device.logical.map_memory(mesh_bundle.staging.memory, staging_offset, size_col, vk::MemoryMapFlags::empty()).unwrap() as *mut [f32; 3];
